...
class Areas extends BaseController {
	...
	function getAllAction(){
		$sql_res = $this->db_link->query("SELECT * FROM `areas` ORDER BY `sort` DESC")->fetch_all(MYSQLI_ASSOC);
		//print_r($sql_res);
	...
	function addAction(){
		...
		$message = "Область (ID = $new_area_id) успешно добавлена";
	...
	function deleteAction(){
		...
		if(isset($_GET['timestamp'])){
			...
			if($live > 10){
				...
				$message = "Область (ID = $id) успешно удалена";
				return json_encode( ['result' => true, 'message' => $message]);
			}
		}	
	}
		...
		if(isset($_GET['timestamp'])){
			...
			if($live > 10){
				...
				if(mysqli_num_rows($sql_q_less_more)>0){
					...
					return json_encode( ['result' => false, 'message' => "Область (ID = $id) уже $where"]);
				}
			}
		}else{
